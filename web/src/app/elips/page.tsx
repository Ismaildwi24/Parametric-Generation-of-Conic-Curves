...
export default function ElipsPage() {
  ...
  const { lowResData, highResData } = useMemo(() => {
    ...
    return { 
      lowResData: generateEllipse(params, deltaThetaLow), 
      highResData: generateEllipse(params, deltaThetaHigh), 
    };
  ...
  const maxRange = isGenerated && a !== '' && b !== '' && centerX !== '' && centerY !== ''
    ? Math.max(a as number + Math.abs(centerX as number), b as number + Math.abs(centerY as number)) + viewPadding
    : 10;
  ...
  return (
    <CurvePageLayout
      title="Elips"
      subtitle="Ellipse — Kurva tertutup dengan eksentrisitas."
      accentColor="#8B5CF6"
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        
        <div className="lg:col-span-4 space-y-6">
          <ParameterCard title="Parameter Input" description="Atur panjang sumbu dan resolusi.">
            <SliderControl label="Semi-axis a" value={a} min={1} max={10} step={0.5} onChange={setA} />
            <SliderControl label="Semi-axis b" value={b} min={1} max={10} step={0.5} onChange={setB} />
      ...
      {isGenerated && highResData && lowResData && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 mt-6">
          ...
          <ResolutionComparison lowResPoints={lowResData.points} highResPoints={highResData.points} deltaLow={deltaThetaLow as number} deltaHigh={deltaThetaHigh as number} viewRange={viewRange} curveColor="#8B5CF6" />
